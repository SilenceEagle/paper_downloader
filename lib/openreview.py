...
def __download_papers_given_divs(driver, divs, save_dir, paper_postfix, time_step_in_seconds=10, downloader='IDM'):
    error_log = []
...
def __get_into_pages_given_number(driver, page_number, pages):
    WebDriverWait(driver, 20).until(
        EC.element_to_be_clickable((By.XPATH, '''//*[@id="accepted-papers"]/nav/ul/li[3]/a''')))
    for page in pages:
        try:
...
def download_nips_papers_given_url(
    ...
    pages = driver.find_elements(By.XPATH, '//*[@id="accepted-papers"]/nav/ul/li')
    current_page = 1
    ind_page = 2  # 0 << ; 1 <
    ...
    while current_page < start_page:
        if total_pages_number < start_page:  # flip pages until seeing the start page
            ...
            __get_into_pages_given_number(driver=driver, page_number=current_page, pages=pages)
    ...
    page = __get_into_pages_given_number(driver=driver, page_number=current_page, pages=pages)
    ...
    while current_page <= total_pages_number:
        ...
        # res = wait.until(EC.presence_of_element_located(
        #     (By.XPATH, '//*[@id="accepted-papers"]/ul/li/h4/a')))
        res = wait.until(EC.presence_of_element_located((By.ID, "accepted-papers")))
        # print("Successful load the website pagination!->", res)
