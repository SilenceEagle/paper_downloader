...
def save_csv(year):
    ...
    :return: True
    """
    with open(f'..\\csv\\ICCV_{year}.csv', 'w', newline='') as csvfile:
        fieldnames = ['title', 'main link', 'supplemental link']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()

        init_url = f'http://openaccess.thecvf.com/ICCV{year}.py'
        if os.path.exists(f'..\\urls\\init_url_ICCV_{year}.dat'):
            with open(f'..\\urls\\init_url_ICCV_{year}.dat', 'rb') as f:
                content = pickle.load(f)
        else:
            # content = urlopen(init_url).read()
            content = open(f'..\\ICCV_{year}.html', 'rb').read()
            with open(f'..\\urls\\init_url_ICCV_{year}.dat', 'wb') as f:
...
def save_csv_workshops(year):
    """
    write ICCV workshops papers' and supplemental material's urls in one csv file
    :param year: int
    :return: True
    """
    with open(f'..\\csv\\ICCV_WS_{year}.csv', 'w', newline='') as csvfile:
        fieldnames = ['group', 'title', 'main link', 'supplemental link']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()

        init_url = f'http://openaccess.thecvf.com/ICCV{year}_workshops/menu'
        if os.path.exists(f'..\\urls\\init_url_ICCV_WS_{year}.dat'):
            with open(f'..\\urls\\init_url_ICCV_WS_{year}.dat', 'rb') as f:
                content = pickle.load(f)
        else:
            content = urlopen(init_url).read()
            # content = open(f'..\\ICCV_WS_{year}.html', 'rb').read()
            with open(f'..\\urls\\init_url_ICCV_WS_{year}.dat', 'wb') as f:
                pickle.dump(content, f)
        soup = BeautifulSoup(content, 'html5lib')
...
def download_from_csv(
    ...
    if is_workshops:
        postfix = f'ICCV_WS_{year}'
    csv_file_name = f'..\\csv\\ICCV_{year}.csv' if not is_workshops else f'..\\csv\\ICCV_WS_{year}.csv'
...
#     error_log = []
#     postfix = f'ICCV_{year}'
#     with open(f'..\\csv\\ICCV_{year}.csv', newline='') as csvfile:
#         myreader = csv.DictReader(csvfile, delimiter=',')
#         pbar = tqdm(myreader)
#         i = 0
#         for this_paper in pbar:
...
if __name__ == '__main__':
    year = 2013
    # total_paper_number = 499
    # # total_paper_number = save_csv(year)
    total_paper_number = save_csv_workshops(year)
    download_from_csv(year,
                      save_dir=f'F:\\workspace\\python3_ws\\ICCV_WS_{year}',
                      is_download_supplement=True,
                      time_step_in_seconds=5,
                      total_paper_number=total_paper_number,
                      is_workshops=True)
    # move_main_and_supplement_2_one_directory_WS(main_path=f'F:\\workspace\\python3_ws\\ICCV_WS_{year}\\main_paper',
    #                                             supplement_path=f'F:\\workspace\\python3_ws\\ICCV_WS_{year}\\supplement')
    # download_from_csv(year,
