...
def save_csv(year):
    """
    write CVPR papers' and supplemental material's urls in one csv file
    :param year: int
    :return: True
    """
...
def download_from_csv(year, save_dir, is_download_supplement=True, time_step_in_seconds=5, total_paper_number=None):
    ...
    with open(f'CVPR_{year}.csv', newline='') as csvfile:
...
if __name__ == '__main__':
    year = 2013
    total_paper_number = 643
    total_paper_number = save_csv(year)
    download_from_csv(year,
                      save_dir=f'..\\CVPR_{year}',
